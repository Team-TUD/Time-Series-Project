...
from alternative_approaches.timegan_ydata.src.ydata_synthetic.synthesizers import gan
...
class TimeGAN(gan.Model):
    def __init__(self, model_parameters, hidden_dim, seq_len, n_seq, gamma, scale_train):
    ...
    def train(self, data, train_steps):
        ...
        for _ in tqdm(range(train_steps * self.scale_train), desc='Emddeding network training'):
        ...
        for _ in tqdm(range(train_steps * self.scale_train), desc='Supervised network training'):
            X_ = next(self.get_batch_data(data, n_windows=len(data)))
    ...
    def sample(self, n_samples):
        ...
        for _ in trange(steps, desc='Synthetic data generation'):
            ...
            print(records)
            print(np.asarray(records).shape)
            data.append(records)
